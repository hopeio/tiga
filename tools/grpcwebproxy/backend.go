...
func buildBackendTlsOrFail() *tls.Config {
	...
	if *flagBackendTlsNoVerify {
		...
		if len(*flagBackendTlsCa) > 0 {
			...
			for _, path := range *flagBackendTlsCa {
				...
				if err != nil {
					log.Fatalf("failed reading backend CA file %v: %v", path, err)
				}
				if ok := tlsConfig.RootCAs.AppendCertsFromPEM(data); !ok {
					log.Fatalf("failed processing backend CA file %v", path)
				}
			}
		}
	}
	if *flagBackendTlsClientCert != "" || *flagBackendTlsClientKey != "" {
		if *flagBackendTlsClientCert == "" {
			log.Fatal("flag 'backend_client_tls_cert_file' must be set when 'backend_client_tls_key_file' is set")
		}
		if *flagBackendTlsClientKey == "" {
			log.Fatal("flag 'backend_client_tls_key_file' must be set when 'backend_client_tls_cert_file' is set")
		}
		cert, err := tls.LoadX509KeyPair(*flagBackendTlsClientCert, *flagBackendTlsClientKey)
		if err != nil {
			log.Fatalf("failed reading TLS client keys: %v", err)
		}
