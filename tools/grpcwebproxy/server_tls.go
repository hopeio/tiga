...
func buildServerTlsOrFail() *tls.Config {
	if *flagTlsServerCert == "" || *flagTlsServerKey == "" {
		log.Fatalf("flags server_tls_cert_file and server_tls_key_file must be set")
	}
	...
	default:
		log.Fatalf("Uknown value '%v' for server_tls_client_cert_verification", *flagTlsServerClientCertVerification)
	}
	if tlsConfig.ClientAuth != tls.NoClientCert {
		if len(*flagTlsServerClientCAFiles) > 0 {
			...
			if err != nil {
				log.Fatalf("no client CA files specified, fallback to system CA chain failed: %v", err)
			}
		}
	...
	if err != nil {
		log.Fatalf("can't configure h2 handling: %v", err)
	}
	return tlsConfig
}
