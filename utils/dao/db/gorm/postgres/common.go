...
const existsByColumnSQL = `SELECT EXISTS(SELECT * FROM %s WHERE %s = ?` + dbconst.WithNotDeleted + ` LIMIT 1)`
const existsSQL = `SELECT EXISTS(SELECT * FROM %s WHERE %s  LIMIT 1)`
const deleteSQL = `Update %s SET deleted_at = now() WHERE %s = ?` + dbconst.WithNotDeleted
...
WHERE id = ?  AND user_id = ?` + dbconst.WithNotDeleted + ` LIMIT 1)`
	var exists bool
