...
type GrpcWebWrapperTestSuite struct {
	...
	wrappedServer    *WrappedGrpcServer
}
...
func TestNonRootResource(t *testing.T) {
	...
	wrappedServer := WrapServer(grpcServer,
		WithAllowNonRootResource(true),
	...
	req := httptest.NewRequest("OPTIONS", "http://host/grpc/improbable.test.TestService/Echo", nil)
	req.Header = headers
...
func (s *GrpcWebWrapperTestSuite) TestPing() {
	...
	for _, contentType := range []string{grpcWebContentType, grpcWebTextContentType} {
		headers, trailers, responses, err := s.makeGrpcRequest(
			"/improbable.test.TestService/Ping",
			headerWithFlag(),
...
func (s *GrpcWebWrapperTestSuite) TestPingList() {
	headers, trailers, responses, err := s.makeGrpcRequest(
		"/improbable.test.TestService/PingList",
		headerWithFlag(),
...
func (s *GrpcWebWrapperTestSuite) TestCORSPreflight_DeniedByDefault() {
	/**
	OPTIONS /improbable.test.TestService/Ping
	...
	*/
	...
	corsResp, err := s.makeRequest("OPTIONS", "/improbable.test.TestService/PingList", headers, nil, false)
...
func (s *GrpcWebWrapperTestSuite) TestCORSPreflight_AllowedByOriginFunc() {
	...
	*/
	...
	corsResp, err = s.makeRequest("OPTIONS", "/improbable.test.TestService/Unknown", headers, nil, false)
	assert.NoError(s.T(), err, "cors preflight should not return errors")
...
func (s *GrpcWebWrapperTestSuite) TestCORSPreflight_EndpointsOnlyTrueWithHandlerFunc() {
	...
	*/
	...
	const pingMethod = "/improbable.test.TestService/PingList"
	const badMethod = "/improbable.test.TestService/Bad"
	...
	s.wrappedServer = WrapHandler(mux,
...
func (s *GrpcWebWrapperTestSuite) assertTrailersContainMetadata(trailers Trailer, meta metadata.MD) {
	for k, v := range meta {
		for _, vv := range v {
...
func (s *GrpcWebWrapperTestSuite) assertTrailerGrpcCode(trailers Trailer, code codes.Code, desc string) {
